...
\end{align}
where \(\hat{F}\) is the dimensionless wing-shape factor. The integral is similarly performed for drag. 
...
Combining into a total force, we then have 
\begin{equation}
    \D \mathbf{F} = \D F_L \mathbf{e}_L + \D F_D \mathbf{e}_D,
\end{equation}
Note that \(\mathbf{e}_L = -\mathbf{e}_{{\omega}_h}\), and \(\mathbf{e}_D = -\mathbf{e}_{v_h}\).  

In the wing-fixed frame, if
\begin{equation}
\boldsymbol{\omega}
=
\omega_x \mathbf e_x
...
Then for a blade element at radial distance \(r\),
\begin{equation}
\mathbf v_h
=
\boldsymbol{\omega}_h \times r\mathbf e_x .
\end{equation}
...
So the unit velocity direction is
\begin{equation}
\mathbf e_{v_h}
=
\frac{
...
\end{equation}
Note that since \(x\), \(y\), and \(z\) are defined as the wing-fixed axes, we need to rotate from the hinge to the Drake wing body frame to the aero/span-chord frame. 

Finally, we just need to apply each \(\D F_L\) and \(\D F_D\) to the blade's \(r_{cp}\). The formula is given by 
\begin{equation}
    % rcp,i​(t)=(ri​+xr​)ex​+ycp,i​(t)ey​.
    r_{cp,i}(t) = (r_i + x_r)\mathbf{e}_x + y_{cp,i}(t)\mathbf{e}_y.
\end{equation}
Again, the wing-fixed axes are defined by the span, chord, and normal axes calculated in the HTML GUI. 

Don't forget to rotate from the wing-fixed Drake frame to the slightly rotated aero/span-chord frame. 
...
\end{equation}
where \(t_h = 0.025\,\mathrm{mm}\), \(w_h = 2.7\,\mathrm{mm}\), and \(L_h = 0.10\,\mathrm{mm}\) are the thickness, width, and length of the central layer. For Kapton, \(E_h = 2.5\,\mathrm{GPa}\).
...
\dot{\omega}_x
\end{equation}
Here, \(\hat{I}_{xy,\mathrm{am}}\) is the added mass analogue of \(I_{xx}\), and is a constant found by the integration here 
\begin{align}
\hat{I}_{xy,am}
&= \int_{0}^{1}
