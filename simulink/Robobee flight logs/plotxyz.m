...
% Log format versions (by number of yout columns):
%   76 cols : 2021-08-05  adaptive attitude only
%   82 cols : 2021-08-10  + adaptive lateral (77-79) + normalized ev (80-82)
%   91 cols : 2023        + reference trajectory r/v/b_1_d (83-91)
n_col = size(yout,2);
n_samples = size(yout,1);

% Inertia matrix (needed by the attitude error loop below)
I_moment = eye(3);
I_moment(1,1) = Ixx;
I_moment(2,2) = Iyy;
I_moment(3,3) = Izz;
...
if n_col >= 82
	...
	normalized_ev_x = nan(n_samples,1);
	normalized_ev_y = nan(n_samples,1);
	normalized_ev_z = nan(n_samples,1);
end
...
if n_col >= 79
	...
	adaptive_lateral_output = nan(n_samples,3);
end
...
% Logged only in the 91-column format. Older logs (hover / setpoint flights)
% fall back to the constant setpoints r_desired, v_desired, b_1_d.

has_reference_traj = n_col >= 91;
if has_reference_traj
	...
	if ~exist('v_desired','var'), v_desired = [0 0 0]; end
	if ~exist('b_1_d','var'),     b_1_d = [1 0 0];     end
	ones_col = ones(n_samples,1);

	r_reference_x = r_desired(1)*ones_col;
	r_reference_y = r_desired(2)*ones_col;
	r_reference_z = r_desired(3)*ones_col;

	v_reference_x = v_desired(1)*ones_col;
	v_reference_y = v_desired(2)*ones_col;
	v_reference_z = v_desired(3)*ones_col;

	b_1_d_reference_x = b_1_d(1)*ones_col;
	b_1_d_reference_y = b_1_d(2)*ones_col;
	b_1_d_reference_z = b_1_d(3)*ones_col;
...
% Adaptive cross-term gains; older logs store them under c_1 / c_2 (or the
% misspelled c_2_adpative), so fall back in that order.
if exist('c_1_adaptive','var'),     c_1 = c_1_adaptive; end
if exist('c_2_adaptive','var'),     c_2 = c_2_adaptive;
elseif exist('c_2_adpative','var'), c_2 = c_2_adpative; end
c_1
c_2
...
figure(12)
plot(time_vicon,normalized_ex_x*upp_bound_x*c_1*50,'r');
hold on;
plot(time_vicon,normalized_ev_x*upp_bound_vx,'r--');
plot(time_vicon,normalized_ev_x*upp_bound_vx+normalized_ex_x*upp_bound_x*c_1*50,'b');
xlabel('Time (s)')
...
% Marker times: two reference periods when a trajectory was flown, otherwise
% the whole controlled window (setpoint / hover logs have no period_ref).
if has_reference_traj && exist('period_ref','var') && exist('total_T','var')
	list_time_end = min(start_delay+period_ref*2, start_delay+total_T);
else
	list_time_end = running_time;
end
list_time = linspace(start_delay, list_time_end, 300);
list_time_index = min(max(floor(list_time*sampling_f),1), n_samples);
...
if has_reference_traj
	...
	% Constant setpoint: one fixed-length heading arrow at the setpoint, and
	% fixed-length body-x arrows along the flown path (autoscale off, since
	% quiver3 cannot autoscale coincident points sensibly).
	arrow_len = 0.02; % m
	quiver3(r_desired(1),r_desired(2),r_desired(3),b_1_d(1)*arrow_len,b_1_d(2)*arrow_len,b_1_d(3)*arrow_len,0,'r','LineWidth',1.5)
	quiver3(x_avg(list_time_index),y_avg(list_time_index),z_avg(list_time_index), heading_avg_e1(list_time_index,1)*arrow_len, heading_avg_e1(list_time_index,2)*arrow_len,heading_avg_e1(list_time_index,3)*arrow_len,0,'b');
end
...
legend('x avg','y avg','z avg',['x ' reference_label],['y ' reference_label],['z ' reference_label],'Location','southeast')
grid on
xlabel('Time (s)')
ylabel('position (m)')
ylim([-0.05,0.2])