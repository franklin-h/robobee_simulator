...
% Rotational aero damping (flapping counter-torque): the real vehicle's
% wings produce a torque opposing body rate, so a rate disturbance decays
% on its own. Model it as e_ds(k+1) = (1 - b*dt)*e_ds(k). Without this the
% attitude block is an undamped double integrator and over-predicts tilt.
% b_rot in 1/s (system-id ~30-70); dt is in ms here, so convert.
b_rot = 50.0;                              % [1/s] rotational damping
rot_decay = 1.0 - b_rot * (dt * 1.0e-3);  % per-step decay factor
if rot_decay < 0.0
    rot_decay = 0.0;                       % guard against b*dt > 1
elseif rot_decay > 1.0
    rot_decay = 1.0;
end
