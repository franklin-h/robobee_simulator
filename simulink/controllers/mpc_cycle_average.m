function [thrust_desired, roll_torque_desired, pitch_torque_desired, yaw_torque_desired] = ...
...
% -------------------------------------------------------------------------
persistent T0 tau_applied tau_cmd_prev
persistent update_phase_ms held_tau_x_t held_tau_y_t have_held_control
...
if isempty(update_phase_ms)
    update_phase_ms = 0.0;
end
if isempty(held_tau_x_t)
    held_tau_x_t = 0.0;
end
if isempty(held_tau_y_t)
    held_tau_y_t = 0.0;
end
if isempty(have_held_control)
    have_held_control = false;
end
...
% -------------------------------------------------------------------------
% Multi-wingbeat command update/hold
%
% The MATLAB Function block still executes every controller_dt, so the
% internal applied-moment lag state is propagated at 0.2 ms. However, a new
% QP solution is sent only once every three wingbeats. Between those update
% instants, thrust and roll/pitch torque commands are held constant.
%
% The phase accumulator avoids long-term timing drift because three
% wingbeats at 155 Hz (19.3548 ms) is not an integer multiple of 0.2 ms.
% -------------------------------------------------------------------------
wingbeat_hz = 155.0;
command_update_wingbeats = 3.0;
command_update_period_ms = command_update_wingbeats * 1.0e3 / wingbeat_hz;

update_due = ~have_held_control || ...
             (update_phase_ms >= command_update_period_ms);

if ~update_due
    % Continue propagating the actuator-lag estimate under the held command.
    beta_c_hold = 1.0 - exp(-controller_dt / tau_lag);
    tau_applied(1) = tau_applied(1) + ...
        beta_c_hold*(held_tau_x_t - tau_applied(1));
    tau_applied(2) = tau_applied(2) + ...
        beta_c_hold*(held_tau_y_t - tau_applied(2));

    update_phase_ms = update_phase_ms + controller_dt;

    % Hold the previously sent total thrust and torque commands.
    thrust_desired = m_t * T0 * 1.0e-3;
    roll_torque_desired  = held_tau_x_t * 1.0e-6;
    pitch_torque_desired = held_tau_y_t * 1.0e-6;
    yaw_torque_desired   = 0.0;

    thrust_desired = clamp_scalar(thrust_desired, 0.0, thrust_max_N);
    roll_torque_desired = clamp_scalar( ...
        roll_torque_desired, -roll_max_Nm, roll_max_Nm);
    pitch_torque_desired = clamp_scalar( ...
        pitch_torque_desired, -pitch_max_Nm, pitch_max_Nm);
    return;
end

% Preserve the residual fraction of a controller step so the average update
% interval remains exactly three wingbeats instead of rounding to 97 calls.
if have_held_control
    update_phase_ms = update_phase_ms - command_update_period_ms;
    if update_phase_ms < 0.0
        update_phase_ms = 0.0;
    end
else
    update_phase_ms = 0.0;
...
U = solve_box_qp_coordinate_descent(H, h, Umin, Umax, nU);
...
% Cache this newly sent command for the next three-wingbeat hold interval.
held_tau_x_t = tau_x_t;
held_tau_y_t = tau_y_t;
have_held_control = true;
update_phase_ms = update_phase_ms + controller_dt;
