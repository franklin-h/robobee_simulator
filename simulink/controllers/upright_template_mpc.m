function [thrust_desired, roll_torque_desired, pitch_torque_desired, yaw_torque_desired] = upright_template_mpc(R_cur,s_des, omega, omega_desired, omega_d_dot,x_d_ddot,e_x,e_v,e_R,e_Omega,control_gain,I_moment_vec, m, g, cntrl_enable,adaptive_pitch, adaptive_yaw, adaptive_x,adaptive_y, adaptive_z)
%#codegen
...
dt = 5.0;       % [ms] prediction step (template default)
ny = 6;
nu = 3;
...
tau_lag = 6.45;        % [ms] moment actuation lag (system-id: dead+lag)
...
ws      = 1.0e1;    % running orientation-vector weight
wds     = 20.0e2;    % running orientation-rate weight
wpr     = 1.0;      % running position weight
wpf     = 3.0;      % final position weight
wvr     = 1.0e3;    % running velocity weight
wvf     = 2.0e3;    % final velocity weight
wthrust = 0.5e-1;   % specific-thrust-correction effort
wmom    = 1.0e-2;    % torque effort


% Actuator limits (SI, same as previous controllers)
thrust_min_N = 0.5e-3;   % [N]
...
Btau = BtauFull(:, 1:2);                      % no yaw torque column
...
% -------------------------------------------------------------------------
beta = dt / tau_lag;          % first-order lag step fraction (Euler)
if beta > 1.0
    beta = 1.0;
end
...
% -------------------------------------------------------------------------
Qrun = [wpr; wpr; wpr; ws; ws; ws; wvr; wvr; wvr; wds; wds; wds; 0; 0];
Qfin = [wpf; wpf; wpf; ws; ws; ws; wvf; wvf; wvf; wds; wds; wds; 0; 0];
