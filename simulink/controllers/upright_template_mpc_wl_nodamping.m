function [pdotdes, h0_wl, accdes, thrust_desired, roll_torque_desired, ...
    pitch_torque_desired, yaw_torque_desired] = upright_template_mpc_wl_nodamping( ...
...
%   the tail of the prediction is soft. NOTE this NODAMPING variant models
%   the attitude-rate states as neutral integrators (no aero damping, no
%   anti-damping) -- see the dynamics section for rationale.
%
...
% NO aero damping model: this variant deliberately omits the per-axis
% rotational damping terms (b_roll/b_pitch from the old open-loop step ID,
% now suspected unreliable). The attitude-rate states are neutral
% integrators -- Ad(10:12,10:12) stays identity -- so the MPC assumes any
% ds persists until actively torqued away. Consequences vs the damped
% variant: it supplies ALL damping actively on every axis (no free-damping
% credit, no anti-damping anticipation on pitch), which is conservative
% and symmetric. If pitch really is anti-damped, expect slightly later /
% larger pitch corrections than upright_template_mpc_wl.m would make.
...
% rows 10:12 (e_ds): identity from eye(nx) -- no modeled decay/growth
...
% the longer-horizon QP (nU = 30); the 1 kHz warm-started replan makes
% each solve's remaining error small.
for iter = 1:30
    for i = 1:nU
        grad_i = h(i);
        for j = 1:nU
